...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!target) {
            return message.channel.send('⚠️ ¡Mencione al usuario al que quieres quitarle la advertencia!')
                .then(msg => setTimeout(() => msg.delete().catch(() => {}), 5000));
        }
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(0x00FF00) // Color verde para indicar éxito/remoción
                .setThumbnail(target.user.displayAvatarURL())
                .setDescription(`✅ Se le ha retirado una advertencia a <@${target.id}>\n\n👤 Acción realizada por: <@${message.author.id}>\n\n🗒️ Razón\n${reason}`)
            ...
            console.error(error);
            message.channel.send('❌ Ocurrió un error al intentar quitar la advertencia.');
        }
    }
};
